...
const CheckOut: NextPageWithLayout = () => {
  ...
  const handleChangeInforCus = (name: string, value: string): void => {
    if (invalidFields.includes(name as keyof IInforCheckout)) {
      const newFields = invalidFields.filter((field: string) => field !== name);
          ...
          <div className="lg:w-6/12 w-full">
            ...
            >
              <div className="flex lg:flex-nowrap flex-wrap w-full items-center justify-between gap-3">
                ...
                <InputText
                  name="phoneNumber"
                  error={invalidFields.includes("phoneNumber") ? true : false}
              ...
              <div className="flex lg:flex-nowrap flex-wrap w-full items-center justify-between gap-3">
                <InputText
                  name="address"
                  error={invalidFields.includes("address") ? true : false}
                  ...
                  {paymentMethods.map(
                    (method: IMethodPayment, index: number) => (
                      <li key={method.id}>
                        ...
                        {selectPayment?.id === 3 && method.id === 3 && (
                          <ul className="px-5 py-2 border-t">
                            ...
                            <li className="lg:text-base text-sm">
                              Sau khi chuyển khoản thành công, chụp màn hình,
                              gửi vào messenger hoặc zalo cho Antran Shop để
              ...
              <Fragment>
                          ...
                          {item.variation && (
                            <p className="sm:text-sm text-xs mt-2">
                              {(item.variation.promotion_price as number) > 0
                                ...
                                : formatBigNumber(
                                    item.variation.price as number
                                  )}
                              {" VND "}X {item.quantity}
                            </p>
                          )}
                      ...
                      {item.variation && (
                        <p className="sm:text-base text-sm font-medium">
                          {(item.variation.promotion_price as number) > 0
                            ...
                            : formatBigNumber(
                                (item.variation.price as number) * item.quantity
                              )}
                          {" VND "}
                        </p>
                      )}
                ...
                <div className="py-5 border-b gap-10">
                  <div className="flex items-center justify-between py-1">
                    ...
                    <p className="text-base">{formatBigNumber(subTotal)} VND</p>
                  </div>
                  ...
                  {coupon && (
                    <div className="flex items-center justify-between py-1">
                      ...
                      <p className="text-base">
                        -{formatBigNumber(subTotal - totalDiscount)} VND
                      </p>
                    </div>
                  )}
